# =============================================================================
# R/core/state.R — Centralized reactive state factories
# =============================================================================
# CHRYSALIS PHASE — contrat d'état (ROADMAP.txt).
...
#
#   - create_sc_shared_state      (moved from R/sc/sc_state.R)
#   - create_spatial_shared_state (new: extracted from global_data$spatial_results_cache schema)
#
# R/sc/sc_state.R is retained as a thin re-export for backward compatibility
# (tests or external code that sources it directly). Canonical definitions live HERE.
# =============================================================================
...
# =============================================================================
# Contrat d'analyse generique (CHRYSALIS 2A) — new_analysis_state() +
# accesseurs state_get()/state_set()/state_has().
#
# Les fabriques ci-dessus decrivent le schema PARTAGE par domaine (consomme
# par les modules existants). new_analysis_state() decrit, lui, l'etat d'UNE
# analyse : les 5 emplacements du contrat transversal (input, preprocessing,
# results, visualization, provenance) — c'est l'ancrage du manifeste de
# provenance (R/core/provenance.R) et le moule des futurs etats par-analyse.
#
...
new_analysis_state <- function(domain = c("sc", "bulk", "spatial")) {
  ...
  if (length(domain) != length(.domains) || !setequal(domain, .domains)) {
    if (length(domain) != 1L || is.na(domain) || !domain %in% .domains) {
      stop(sprintf(
        "new_analysis_state() : domaine '%s' inconnu (autorisés : sc, bulk, spatial).",
...
#'
#' @param state reactiveValues (fabrique de domaine ou new_analysis_state()).
