...
mod_sc_da_design_ui <- function(id) {
  ...
  tagList(
    ...
    selectInput(ns("da_condition_col"), i18n$t("Colonne condition (requis)"),
                choices = character(0), width = "100%"),
    # V1.x UX multi-échantillons : exemple concret de structure attendue,
    # collé aux sélecteurs sample_col / condition_col (anti pseudo-réplication).
    div(class = "alert alert-light", style = "font-size:0.8em;border-left:3px solid #F39C12;",
        tags$b(i18n$t("Exemple de structure de métadonnées attendue (4 échantillons, 2 conditions) :")),
        tags$pre(class = "mb-0", style = "font-size:0.75em;margin-top:4px;margin-bottom:0;",
                 i18n$t("condition : Control, Control, Treatment, Treatment")),
        tags$pre(style = "font-size:0.75em;margin-top:2px;margin-bottom:4px;",
                 i18n$t("sample_id : S1, S2, S3, S4")),
        tags$span(bsicons::bs_icon("exclamation-triangle"), " ",
                  i18n$t("Les cellules ne sont JAMAIS des réplicats biologiques : sample_id est l'unité de réplication."))),
